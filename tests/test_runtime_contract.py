...
class RuntimeContractTests(unittest.TestCase):
    ...
    def test_default_html_contract_does_not_require_mobile_adaptation(self) -> None:
        contract = self.init_contract("--visual", "retained")
        ...
        self.assertIn("html-accessible-print-layout", required_rules)
        self.assertIn("visual-stable-layout", required_rules)
        self.assertNotIn("html-responsive-accessible-print", required_rules)
        self.assertNotIn("visual-responsive-layout", required_rules)
        for mobile_requirement in ("390px", "mobile render", "mobile screenshot"):
            self.assertNotIn(mobile_requirement, bundle_text)
    ...
    def test_html_validator_accepts_document_without_viewport_meta(self) -> None:
        html = self.base / "desktop-report.html"
        html.write_text(
            "<!-- Generated by Reader's Seat -->\n"
            '<!doctype html><html lang="en"><head><title>Desktop report</title></head>'
            '<body><main><h1>Desktop report</h1><p>Verified desktop content.</p>'
            "</main></body></html>\n",
            encoding="utf-8",
        )
        validation = subprocess.run(
            [sys.executable, "-B", str(HTML_VALIDATOR), str(html)],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
